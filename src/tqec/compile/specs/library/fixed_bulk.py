...
from tqec.compile.specs.library.generators.y_basis_fixed_bulk import (
...
class FixedBulkCubeBuilder(CubeBuilder):
    ...
    @functools.cache
    def _call_impl(
        self, spec: CubeSpec, block_temporal_height: LinearFunction
    ) -> Block:
        kind = spec.kind
        ...
        layers: list[BaseLayer | BaseComposedLayer] = [
            ...
            RepeatedLayer(
                PlaquetteLayer(template, repeat), repetitions=block_temporal_height
            ),
