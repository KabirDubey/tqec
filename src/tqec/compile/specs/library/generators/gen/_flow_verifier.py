...
class FlowStabilizerVerifier:
    def __init__(self, next_measurement: int, q2i: Dict[complex, int], flows: Iterable[Flow]):
        ...
        self.xs = np.zeros(shape=(num_qubits, len(self.flows)), dtype=np.bool)
        self.zs = np.zeros(shape=(num_qubits, len(self.flows)), dtype=np.bool)
