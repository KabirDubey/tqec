...
def _is_transition_temporal_border(layer: BaseLayer) -> bool:
    ...
    if not isinstance(layer, PlaquetteLayer):
        return False
    return isinstance(layer.template, _YRoundTemplate) and layer.template.kind == "switch"
...
class TopologicalComputationGraph:
    ...
    def layout_positions(self) -> set[LayoutPosition3D]:
        ...
        return set(self._layered_blocks.keys())
    ...
    def _check_any_pipe(self, source: BlockPosition3D, sink: BlockPosition3D) -> None:
        ...
        if source_layout_position not in self.layout_positions:
            raise TQECError(
        ...
        if sink_layout_position not in self.layout_positions:
            raise TQECError(
    ...
    def _replace_temporal_borders(
        ...
        if not self._temporal_border_is_transition(source, TemporalBlockBorder.Z_POSITIVE):
        ...
        if not self._temporal_border_is_transition(sink, TemporalBlockBorder.Z_NEGATIVE):
            self._replace_temporal_border(
                sink,
    ...
    def _temporal_border_is_transition(
        ...
        block = self._layered_blocks.get(LayoutPosition3D.from_block_position(block_pos))
        if block is None:
            return False
        try:
